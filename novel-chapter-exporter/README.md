...
- Agent 只能读取项目工作区和本次小说输出目录（`保存位置/小说名`），不能读取整个保存位置；只能写入 `agent_workspace/memory`，不能写到其他位置。
- Agent 不能执行模型生成的 Python/PowerShell 脚本；受限命令只允许运行安全参数的 `rg`，会拒绝 `--pre`、`--pre-glob`、`--search-zip` 等会触发外部处理器或扩大读取面的参数。
- Agent 可写入和运行声明式 JSON 脚本（`.agent.json`），由程序解释执行；只支持 `list_dir`、`read_text`、`search_text`、`parse_json`、`regex_extract`、`count_chapter_files`、`write_memory` 等受控操作。
...
- Agent 可用 `run_limited_command` 执行受限命令：只允许安全参数的 `rg`；不能任意执行 Python、PowerShell 或 cmd。
- 日志区提供“复制日志”按钮。
