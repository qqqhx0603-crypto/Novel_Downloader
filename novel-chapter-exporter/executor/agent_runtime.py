...
10. 你可读项目工作区和用户在 GUI 中选择的下载目录；可写 agent_workspace/memory；受限命令只能通过 run_limited_command，且只允许运行 rg。
...
TOOLS = [
    ...
    },
    {
        ...
        "function": {
            ...
            "description": "在受限命令行运行 rg；cwd 只能是项目目录或 GUI 选择的下载目录，不允许运行模型生成的 Python 脚本。",
            "parameters": {
                "type": "object",
                "properties": {
                    "argv": {
...
def _trusted_output_args(args: dict[str, Any], trusted_output_dir: str) -> dict[str, Any]:
    if not trusted_output_dir:
        raise RuntimeError("缺少 GUI 保存路径，不能执行需要下载目录授权的工具。")
    safe_args = dict(args)
    safe_args["output_dir"] = trusted_output_dir
    return safe_args
...
def _tool_result_payload(
    ...
    if name == "download_novel":
        args = _trusted_output_args(args, trusted_output_dir)
...
def run_agent(form_state: dict[str, Any], log: Callable[[str], None] = print) -> dict[str, Any]:
    ...
    for step in range(1, max_steps + 1):
        ...
        for call in tool_calls:
            ...
            try:
                result = _tool_result_payload(name, args, log, trusted_output_dir)
