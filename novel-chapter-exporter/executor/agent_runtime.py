...
2. 不要虚构下载结果；下载必须通过 download_novel 工具，目录状态必须通过 inspect_download_output/list_limited_dir/read_limited_file/run_limited_command/run_agent_script 等工具确认。
...
10. 你可读项目工作区和本次小说输出目录；可写 agent_workspace/memory；可写并运行声明式 JSON agent script，但不能运行 Python/PowerShell/系统脚本；受限命令只能通过 run_limited_command，且只允许运行安全参数的 rg。
...
TOOLS = [
        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    ...
                    "output_dir": {"type": "string", "description": "由 GUI 保存路径强制覆盖；模型提供的值会被忽略。"},
    ...
    },
    {
        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    "path": {"type": "string"},
                    "novel_name": {"type": "string", "description": "小说名；用于把下载目录授权限制到保存位置/小说名"},
                    "output_dir": {"type": "string", "description": "由 GUI 保存路径和小说名强制覆盖；模型提供的值会被忽略。"},
                },
                "required": ["path"],
            },
        },
    ...
    },
    {
        ...
        "function": {
            ...
            "description": "写入 agent 自己的记忆文件，只能写到 agent_workspace/memory。",
            "parameters": {
                "type": "object",
                "properties": {
    ...
    },
    {
        ...
        "function": {
            ...
            "description": "在受限命令行运行 rg；cwd 只能是项目目录或本次小说输出目录，不允许运行模型生成的 Python/PowerShell 脚本，也不能使用 rg --pre。",
            "parameters": {
                "type": "object",
                "properties": {
                    "argv": {
    ...
    },
    {
        "type": "function",
        "function": {
            "name": "write_agent_script",
            "description": "写入声明式 JSON agent script。它不是 Python/PowerShell，只能包含受控 op。",
            "parameters": {
                "type": "object",
                "properties": {
                    "relative_path": {"type": "string", "description": "脚本相对路径，建议以 .agent.json 结尾"},
                    "script": {
                        "type": "object",
                        "description": "格式: {\"steps\":[{\"op\":\"list_dir|read_text|search_text|parse_json|regex_extract|count_chapter_files|write_memory\", ...}]}",
                    },
                },
                "required": ["relative_path", "script"],
            },
        },
    },
    {
        ...
        "function": {
            ...
            "description": "运行声明式 JSON agent script。脚本由程序解释执行，只能访问项目目录和本次小说输出目录。",
            "parameters": {
                "type": "object",
                "properties": {
                    "relative_path": {"type": "string"},
...
def _trusted_output_args(
    args: dict[str, Any],
    trusted_output_dir: str,
    *,
    scope: str,
    fallback_novel_name: str = "",
) -> dict[str, Any]:
    if not trusted_output_dir:
        raise RuntimeError("缺少 GUI 保存路径，不能执行需要下载目录授权的工具。")
    safe_args = dict(args)
    if scope == "book":
        novel_name = str(safe_args.get("novel_name") or fallback_novel_name or "novel").strip()
        safe_book = tools_impl.safe_name(novel_name, "novel")
        safe_args["output_dir"] = str(Path(trusted_output_dir).expanduser().resolve() / safe_book)
    elif scope == "base":
        safe_args["output_dir"] = str(Path(trusted_output_dir).expanduser().resolve())
    else:
        raise RuntimeError(f"未知输出授权范围: {scope}")
    return safe_args
...
def _tool_result_payload(
    ...
    if name == "download_novel":
        args = _trusted_output_args(args, trusted_output_dir, scope="base", fallback_novel_name=fallback_novel_name)
    ...
    if name == "list_limited_dir":
        args = _trusted_output_args(args, trusted_output_dir, scope="book", fallback_novel_name=fallback_novel_name)
        args.pop("novel_name", None)
    ...
    if name == "write_agent_script":
        return tools_impl.write_agent_script(**args)
    if name == "run_agent_script":
        args = _trusted_output_args(args, trusted_output_dir, scope="book", fallback_novel_name=fallback_novel_name)
        args.pop("novel_name", None)
        return tools_impl.run_agent_script(**args)
...
CRITICAL_TOOL_NAMES = {
    ...
    "inspect_novel_source",
    "inspect_novel_catalog",
    "inspect_download_output",
    "test_source_config",
    "register_source_config",
    "run_agent_script",
}
NON_OK_STATUSES = {"error", "partial", "no_chapters_found", "not_found", "invalid", "needs_user", "command_failed"}


def _failure_subject(name: str, args: dict[str, Any], result: dict[str, Any]) -> str:
    return str(
        result.get("book_name")
        or args.get("novel_name")
        or result.get("novel_name")
...
def _failure_key(name: str, args: dict[str, Any], result: dict[str, Any]) -> str:
    return f"{name}:{_failure_subject(name, args, result)}"


def _update_unresolved_failures(
    ...
    *,
    ...
    if name not in CRITICAL_TOOL_NAMES:
        return
    status = str(result.get("status") or "error")
    key = _failure_key(name, args, result)
    ...
    if status == "ok":
        ...
        if name == "download_novel":
            subject = _failure_subject(name, args, result)
            for existing_key in list(unresolved):
                if existing_key.endswith(":" + subject):
                    unresolved.pop(existing_key, None)
...
def run_agent(form_state: dict[str, Any], log: Callable[[str], None] = print) -> dict[str, Any]:
    ...
    if model not in AVAILABLE_MODELS:
        model = DEFAULT_MODEL
    trusted_output_dir = str(form_state.get("output_dir") or "").strip()
    fallback_novel_name = str(form_state.get("novel_name") or "").strip()
    ...
    unresolved_tool_failures: dict[str, dict[str, Any]] = {}
    ...
    for step in range(1, max_steps + 1):
        ...
        if not tool_calls:
            ...
            if unresolved_tool_failures:
                failed = next(iter(unresolved_tool_failures.values()))
        ...
        for call in tool_calls:
            ...
            try:
                result = _tool_result_payload(name, args, log, trusted_output_dir, fallback_novel_name)
    ...
    return {
        ...
        "last_result": next(iter(unresolved_tool_failures.values()), last_result),
        "ui_notifications": ui_notifications,
    }
